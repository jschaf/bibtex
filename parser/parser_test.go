...
func TestParseFile_BibDecl_ModeParseStrings(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "article key with no braced accent",
			src:    `@article { cite_key, key = {Anna H\"af} }`,
			keysFn: asts.WithBibKeys("cite_key"),
			tagsFn: asts.WithBibTags("key", asts.BraceTextExpr(0,
				asts.Text("Anna"),
				&ast.TextSpace{},
				asts.Text("H"),
				asts.AccentedText(token.AccentUmlaut, "a"),
				asts.Text("f"),
			)),
		},
		{
			...
			tagsFn: asts.WithBibTags("key", asts.BraceTextExpr(0,
				...
				&ast.TextSpace{},
				asts.Text("H"),
				asts.BraceText(1, asts.AccentedText(token.AccentUmlaut, "a")),
				asts.Text("user"),
			)),
		},
		{
